...
class DeleteClientControllerTest extends FunctionalTestCase
{
    ...
    public function testDeleteClientNoContentResponse(): void
    {
        ...
        $this->insert(ClientDoctrineRepository::tableName(), $clientData);

        $response = $this->httpDelete('delete-client', ['id' => $clientData[ClientId::primitiveName()]]);

        $this->assertEquals(Response::HTTP_NO_CONTENT, $response->getStatusCode());

        $storedClient = $this->findById(ClientDoctrineRepository::tableName(), new ClientId($clientData[ClientId::primitiveName()]));
    ...
    protected function setUp(): void
    {
        ...
        $this->deleteAll(ClientDoctrineRepository::tableName());
    }
}
